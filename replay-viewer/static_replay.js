// Cogiavelli static replay shell: fetches the replay named by ?replay=<url>,
...
(function () {
  ...
  function start(module, bytes) {
    ...
    var ok = module._cog_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._cog_error_ptr(),
        module._cog_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._cog_payload_ptr(),
        module._cog_payload_len())
    );
    var loading = document.getElementById("loading");
    if (loading) loading.style.display = "none";
    document.documentElement.removeAttribute("data-replay-error");
    CogiavelliRenderer.attachReplay({
    ...
    // The renderer draws on its own animation frame and sets
    // data-replay-loaded="true" at the end of the frame in which it first
    // draws. Report "ready" only AFTER that attribute appears: posting it on
    // rAF timing at the call site lets an embedding page sample an unpainted
    // shell (chorus 3c11c953, 2026-08-24).
    (function waitForFirstFrame(attempts) {
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      modulePromise = CogiavelliReplayModule().catch(function (error) {
