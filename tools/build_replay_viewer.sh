...
# `coworld build` pre-creates the output directory's parent; a plain CI
# checkout does not, and the containment check below would then fail on a
# path that simply does not exist yet (ecos, 2026-08-23).
...
test -s "${dist}/cogiavelli_replay.wasm"
test -s "${dist}/cogiavelli_replay.js"

rm -rf "${output_dir}"
mkdir -p "${output_dir}/assets"
cp "${dist}/cogiavelli_replay.js" "${dist}/cogiavelli_replay.wasm" \
  "${output_dir}/"
cp "${repo_dir}/replay-viewer/index.html" \
  ...
  "${output_dir}/"
for asset in italy1499.json purse.png dagger.png die.png \
  soldier_red_front.png soldier_blue_front.png soldier_green_front.png \
  soldier_yellow_front.png arena_floor.png font.ttf; do
  cp "${repo_dir}/data/${asset}" "${output_dir}/assets/"
done

test -f "${output_dir}/index.html"
test -s "${output_dir}/assets/italy1499.json"
grep -q 'data-replay' "${output_dir}/static_replay.js"
echo "cogiavelli replay viewer bundle: ${output_dir}"
